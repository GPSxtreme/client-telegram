...
export class MessageManager {
	...
	constructor(bot: Telegraf<Context>, runtime: IAgentRuntime) {
		...
		this.autoPostConfig = {
			...
			mainChannelId:
				this.runtime.character.clientConfig?.telegram?.autoPost?.mainChannelId,
	...
	private async _monitorPinnedMessages(ctx: Context): Promise<void> {
		...
		if (
			!this.autoPostConfig.pinnedMessagesGroups.includes(ctx.chat.id.toString())
		)
			return;
	...
	private _isRelevantToTeamMember(
		...
		if (this._isTeamLeader() && lastAgentMemory?.content.text) {
			...
			return (
				similarity >= MESSAGE_CONSTANTS.DEFAULT_SIMILARITY_THRESHOLD_FOLLOW_UPS
			);
		}
	...
	private _checkInterest(chatId: string): boolean {
		...
		if (this._isTeamLeader() && chatState.messages.length > 0) {
			if (
				!this._isRelevantToTeamMember(lastMessage?.content.text || "", chatId)
			) {
			...
			if (
				...
				if (chatState?.messages?.length) {
					...
					const otherTeamMemberResponded = recentResponses.some(
						(m) =>
							m.userId !== this.runtime.agentId && this._isTeamMember(m.userId),
					);
